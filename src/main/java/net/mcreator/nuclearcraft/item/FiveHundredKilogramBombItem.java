...
public class FiveHundredKilogramBombItem extends Item {
    ...
    public FiveHundredKilogramBombItem(Properties properties) {
        super(properties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (!level.isClientSide) {
            ...
            FiveBombEntity bomb = BigExplosivesModEntities.FIVE_BOMB.get().create(level);
            if (bomb != null) {
    ...
    private static Vec3 resolveSpawnPosition(Level level, Player player, double verticalOffset) {
        HitResult hitResult = player.pick(192.0D, 0.0F, false);
