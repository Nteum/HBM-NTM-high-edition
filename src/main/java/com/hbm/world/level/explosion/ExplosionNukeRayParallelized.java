...
public class ExplosionNukeRayParallelized implements IExplosionRay {
	...
	private List<SubChunkKey> getAllSubChunks() {
		...
		int minExplosionY = Math.max(this.minY, origin.getY() - radius);
		int maxExplosionY = Math.min(this.minY + this.worldHeight - 1, origin.getY() + radius);
		int minSectionY = level.getSectionIndex(minExplosionY);
		int maxSectionY = level.getSectionIndex(maxExplosionY);
		int originSectionY = level.getSectionIndex(origin.getY());
	...
	public void destructionTick(int timeBudgetMs) {
		...
		while (it.hasNext() && System.nanoTime() < deadline) {
			...
			if (bs.isEmpty()) {
				...
				for (int sectionY = 0; sectionY < this.sectionsCount; sectionY++) snapshots.remove(new SubChunkKey(cp, sectionY));
				it.remove();
			}
		}
	...
	private class RayTask {
		...
		void init() {
			...
			this.tMaxX = (stepX == 0) ? Double.POSITIVE_INFINITY : tDeltaX * (stepX > 0 ? 1.0 - (px - x) : (px - x));
			...
			this.tMaxY = (stepY == 0) ? Double.POSITIVE_INFINITY : tDeltaY * (stepY > 0 ? 1.0 - (py - y) : (py - y));
			...
			this.tMaxZ = (stepZ == 0) ? Double.POSITIVE_INFINITY : tDeltaZ * (stepZ > 0 ? 1.0 - (pz - z) : (pz - z));
