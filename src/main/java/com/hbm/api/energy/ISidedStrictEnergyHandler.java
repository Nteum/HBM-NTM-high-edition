...
public interface ISidedStrictEnergyHandler extends IStrictEnergyHandler {
     ...
     *
     * <p>
     * <strong>IMPORTANT:</strong> This {@link long} <em>MUST NOT</em> be modified. This method is not for altering internal contents. Any implementers who are
     ...
     * @param side      The side we are interacting with the handler from (null for internal).
     *
     * @return Energy in a given container. 0L if the container has no energy stored.
     */
     ...
     * @param energy    Energy to set the container to (may be 0L).
     * @param side      The side we are interacting with the handler from (null for internal).
     *
     ...
     * @param side      The side we are interacting with the handler from (null for internal).
     *
     * @return The remaining energy that was not inserted (if the entire amount is accepted, then return 0L). The returned {@link long} can
     ...
     * <p>
     * The returned value must be 0L if nothing is extracted, otherwise its must be less than or equal to {@code amount}.
     * </p>
     *
     ...
     * @param side      The side we are interacting with the handler from (null for internal).
     *
     * @return Energy extracted from the container, must be 0L if no energy can be extracted. The returned {@link long} can be safely
    ...
    default long insertEnergy(long amount, @Nullable Direction side, Action action) {
        return LongTransferUtils.insert(amount, action, () -> getEnergyContainerCount(side), container -> getEnergy(container, side),
    ...
    default long extractEnergy(long amount, @Nullable Direction side, Action action) {
        return LongTransferUtils.extract(amount, action, () -> getEnergyContainerCount(side), (container, a, act) -> extractEnergy(container, a, side, act));
    }
}