            ...
            switch (line[0])
            {
                ...
                case "f": // Face
                {
                    ...
                    for (int i = 0; i < vertices.length; i++)
                    {
                        ...
                        // 空索引（如 f v//vn 或缺 vt）用 -1 标记：-1 表示"无此分量"，渲染时用默认值
                        int[] vertex = new int[vertexParts.length];
                        for (int j = 0; j < vertexParts.length; j++){
                            if (Strings.isNullOrEmpty(vertexParts[j])) vertex[j] = -1;
                            else vertex[j] = Integer.parseInt(vertexParts[j]);
                        }
                        if (vertex[0] < 0) vertex[0] = model.positions.size() + vertex[0];
                        else vertex[0]--;
                        if (vertex.length > 1)
                        {
                            if (vertex[1] == -1) vertex[1] = 0; // 无 UV 时使用默认坐标
                            else if (vertex[1] < 0) vertex[1] = model.texCoords.size() + vertex[1];
                            else vertex[1]--;
                            if (vertex.length > 2)
                            {
                                if (vertex[2] == -1) vertex[2] = 0; // 无法线时由 makeQuad 重算
                                else if (vertex[2] < 0) vertex[2] = model.normals.size() + vertex[2];
                                else vertex[2]--;
                                if (vertex.length > 3)
                                {
