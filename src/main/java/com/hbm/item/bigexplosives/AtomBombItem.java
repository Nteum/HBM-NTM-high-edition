...
public class AtomBombItem extends Item {
    ...
    public AtomBombItem(final Properties properties) {
        super(properties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(final Level level, final Player player, final InteractionHand hand) {
        ...
        if (!level.isClientSide) {
            ...
            final AtomicBombEntity bomb = BigExplosivesModEntities.ATOMIC_BOMB.get().create(level);
            if (bomb != null) {
    ...
    private static Vec3 resolveSpawnPosition(final Level level, final Player player, final double verticalOffset) {
        final HitResult hitResult = player.pick(256.0D, 0.0F, false);
