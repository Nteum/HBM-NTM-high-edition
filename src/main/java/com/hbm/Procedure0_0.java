...
//用于加载obj模型
public class Procedure0_0 {
    public static void procedure0_0(){

    }

    public static class CustomerObjModel extends SimpleUnbakedGeometry<CustomerObjModel>{
        ...
        public CustomerObjModel(@NotNull ResourceLocation modelLocation,
        ...
        public static CustomerObjModel parse(String path) throws IOException {
            return parse(path,new ResourceLocation(HBMxx.MODID,path),true,true,true,false,null);
        }

        public static CustomerObjModel parse(String path,
                                             @NotNull ResourceLocation modelLocation,
                                             boolean automaticCulling,
                                             boolean shadeQuads,
                                             boolean flipV,
                                             boolean emissiveAmbient,
                                             @Nullable String mtlOverride) throws IOException {
            String objPath = Procedure0.modAssetsPrefix + path + ".obj";
            String objFolder = new File(objPath).getParent();

            String mtlPath;
            if (mtlOverride != null)mtlPath = mtlOverride;
//            Resource resource = Procedure0.getAssetsResource(path + ".obj");

            CustomerObjModel model = new CustomerObjModel(modelLocation, automaticCulling, shadeQuads, flipV, emissiveAmbient, mtlOverride);
            ...
            if (mtlOverride != null)
            {
                mtllib = loadMtl(objFolder, mtlOverride);
            }
            try(ObjTokenizer tokenizer = new ObjTokenizer(Procedure0.loadAssets(objPath))) {
                while ((line = tokenizer.readAndSplitLine(true)) != null)
                {
                    switch (line[0])
                    {
                        ...
                        case "v": // Vertex 顶点
                            ...
                            break;
                        case "vt": // Vertex texcoord 顶点对应的纹理坐标
                            model.texCoords.add(parseVector2(line));
                            break;
                        case "vn": // Vertex normal 顶点法线
                            model.normals.add(parseVector3(line));
                            break;
                        case "vc": // Vertex color (non-standard) 顶点颜色（未归一化）
                            model.colors.add(parseVector4(line));
                            break;

                        case "f": // Face 面
                        {
                            //ModelObject -> (ModelGroup) -> ModelMesh
                            if (currentMesh == null)
                            {
        ...
        public static ObjMaterialLibrary loadMtl(String folder, String path){
//            Resource resource = Procedure0.getResource(folder, path);
            try (ObjTokenizer rdr = new ObjTokenizer(Procedure0.loadAssets(folder + "/" + path)))
            {
