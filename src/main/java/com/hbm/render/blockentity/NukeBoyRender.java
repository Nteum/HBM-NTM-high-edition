...
public class NukeBoyRender extends MultiPartRenderer<NukeBombBoyEntityBE> {
...
//        bomb_model = modelManager.getModel(Models.BOY);
    ...
    @Override
    public void renderMultiPart(NukeBombBoyEntityBE pBlockEntity, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, int pPackedOverlay) {
        BlockState blockState = pBlockEntity.getBlockState();
        BlockRenderDispatcher blockDispatcher = Minecraft.getInstance().getBlockRenderer();
        ModelBlockRenderer blockRenderer = blockDispatcher.getModelRenderer();
        if (bomb_model == null) bomb_model = blockDispatcher.getBlockModel(blockState);
