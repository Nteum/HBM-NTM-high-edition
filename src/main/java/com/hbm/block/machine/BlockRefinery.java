...
/**
 * 炼油厂（多方块）。移植自旧版 MachineRefinery（8 格高）。
 */
public class BlockRefinery extends BlockDummyable {
    public static MultiblockData MULTIBLOCK_DATA;
    public BlockRefinery(Properties pProperties){
        super(pProperties);
        this.shape = Block.box(-16.0D, 0.0D, -16.0D, 32.0D, 128.0D, 16.0D);
    }

    @Nullable
    @Override
    protected BlockEntity mainBlockEntity(BlockPos pos, BlockState state) {
        return new RefineryEntity(pos, state);
    }

    @Override
    public MultiblockData getMultiblockData() {
        if (MULTIBLOCK_DATA == null){
            MULTIBLOCK_DATA = new MultiblockData(8, 0, 1, 1, 1, 1)
