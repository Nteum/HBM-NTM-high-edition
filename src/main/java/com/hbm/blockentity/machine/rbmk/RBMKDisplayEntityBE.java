 ...
 */
public class RBMKDisplayEntityBE extends BEUpdateable implements RBMKLinkable {
    ...
    private final RBMKPeripheralEntityBE.ConsoleColumn[] columns = new RBMKPeripheralEntityBE.ConsoleColumn[GRID_SIZE * GRID_SIZE];
    ...
    public RBMKDisplayEntityBE(BlockPos pos, BlockState state) {
        super(HBMTiles.RBMK_DISPLAY_ENTITY.get(), pos, state);
    }
    ...
    protected void onUpdateServer() {
        ...
        for (int index = 0; index < columns.length; index++) {
            ...
            columns[index] = state != null ? RBMKPeripheralEntityBE.ConsoleColumn.capture(serverLevel, state) : null;
        }
        sendUpdatePacket();
    }
    ...
    public CompoundTag getReducedUpdateTag() {
        ...
        for (int i = 0; i < columns.length; i++) {
            RBMKPeripheralEntityBE.ConsoleColumn column = columns[i];
            if (column != null) {
        ...
        if (tag.contains("Columns", net.minecraft.nbt.Tag.TAG_COMPOUND)) {
            ...
            for (int i = 0; i < columns.length; i++) {
                ...
                if (gridTag.contains(key, net.minecraft.nbt.Tag.TAG_COMPOUND)) {
                    columns[i] = RBMKPeripheralEntityBE.ConsoleColumn.fromTag(gridTag.getCompound(key));
                }
            }
        }
    }
    ...
    @Nullable
    public RBMKPeripheralEntityBE.ConsoleColumn getConsoleColumn(int index) {
    ...
    private boolean isColumnValid(BlockPos pos) {
        return level != null && level.getBlockEntity(pos) instanceof RBMKBaseEntityBE;
    }
        ...
        for (int dx = -AUTO_LINK_RADIUS; dx <= AUTO_LINK_RADIUS; dx++) {
            for (int dy = -1; dy <= 1; dy++) {
                for (int dz = -AUTO_LINK_RADIUS; dz <= AUTO_LINK_RADIUS; dz++) {
                    ...
                    if (!(blockEntity instanceof RBMKPeripheralEntityBE peripheral)) {
                        continue;
                    }
