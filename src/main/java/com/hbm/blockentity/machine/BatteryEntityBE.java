...
public class BatteryEntityBE extends BaseMachineBE {
...
//    private boolean reloadFlag = false;
    ...
    public BatteryEntityBE(BlockPos pPos, BlockState pBlockState) {
    ...
    protected void onUpdateServer() {
        this.getCapability(HBMCaps.LONG_ENERGY, null).ifPresent(ienergyhandler -> reloadTimer = ((SidedEnergyHandler) ienergyhandler).checkNeighbourIfLoad(this.getLevel(), this.worldPosition) ? 3 : reloadTimer);
        if (reloadTimer > 0){
            reloadTimer--;
            energyReload();
        }
    ...
    private void energyReload(){
        this.getCapability(HBMCaps.LONG_ENERGY, null).ifPresent(ienergyhandler -> ((SidedEnergyHandler) ienergyhandler).rebuildSideData(this.getLevel(), this.worldPosition));
    }
