...
/** 盖革计数器 */
public class TileEntityGeigerBE extends BECapabilities {
    int timer = 0;
    float ticker = 0;
    public TileEntityGeigerBE(BlockPos pPos, BlockState pBlockState) {
        super(pPos, pBlockState);
    }

    public static void ticker(Level level, BlockPos pPos, BlockState pState, BlockEntity pBlockEntity) {
        if (pBlockEntity instanceof TileEntityGeigerBE geiger && !level.isClientSide()){
            geiger.timer++;
            ...
            if(geiger.timer == 10) {
                ...
                level.updateNeighborsAtExceptFromFacing(geiger.worldPosition, geiger.getBlockState().getBlock(), null);
            }
