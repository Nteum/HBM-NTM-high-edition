...
//                return entry.type.getMethod("deserializeNBT", CompoundTag.class).invoke(entry.type.newInstance(), nbt);
    ...
    public static double getDouble(ItemStack itemStack, String key, double defaultValue){
        return itemStack.hasTag() ? getDouble(itemStack.getTag(), key, defaultValue) : defaultValue;
    }
    ...
    public static String getStr(ItemStack itemStack, String key, String defaultValue){
        return itemStack.hasTag() ? getStr(itemStack.getTag(), key, defaultValue) : defaultValue;
    }
    public static String getStr(CompoundTag tag, String key, String defaultValue){
        return tag.contains(key, Tag.TAG_STRING) ? tag.getString(key) : defaultValue;
    }

    public static <E extends Enum> E getEnum(ItemStack itemStack, String key, E defaultValue, Class<E> clazz){
        return itemStack.hasTag() ? getEnum(itemStack.getTag(), key, defaultValue, clazz) : defaultValue;
    }

    public static <E extends Enum> E getEnum(CompoundTag tag, String key, E defaultValue, Class<E> clazz){
        return getList(tag, key, defaultValue, Arrays.asList(clazz.getEnumConstants()));
    }

    public static <T> T getList(ItemStack stack, String key, T defaultValue, List<T> list){
        return stack.hasTag() ? getList(stack.getTag(), key, defaultValue, list) : defaultValue;
    }

    public static <T> T getList(CompoundTag tag, String key, T defaultValue, List<T> list){
        int intValue = getInt(tag, key, -1);
        return intValue >= 0 && intValue < list.size() ? list.get(intValue) : defaultValue;
    }
}