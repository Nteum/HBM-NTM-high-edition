...
import com.hbm.capabilities.network.network.DynamicBufferedNetwork;
...
/**
 * from:mek
 * 可以存储一定内容的transmitter，
 * 额外加入了一个buffer的设定，但并没有在这个类中规定buffer
 * */
public abstract class BufferedTransmitter<ACCEPTOR, NETWORK extends DynamicBufferedNetwork<ACCEPTOR, NETWORK, BUFFER, TRANSMITTER>, BUFFER,
    ...
    public BufferedTransmitter(TransmitterBlockEntity tile, TransmissionType... transmissionTypes) {
    ...
    //获取buffer，但如果没有则获得网络本身的buffer
    @NotNull
    public abstract BUFFER getBufferWithFallback();

    //获取buffer，如果没有则返回null
    ...
    private void recheckConnectionPrechecked(Direction side) {
        TransmitterBlockEntity otherTile = WorldUtils.getTileEntity(TransmitterBlockEntity.class, getTileWorld(), getTilePos().relative(side));
        if (otherTile != null) {
